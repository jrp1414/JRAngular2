"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var Book_1 = require("./Book");
var BookComponent = (function () {
    function BookComponent() {
        this.year = Book_1.Book2.Year;
        this.book = new Book_1.Book2();
        this.Cities = [
            { Name: Book_1.Cities[Book_1.Cities.Pune], Value: Book_1.Cities.Pune }, Book_1.Cities[Book_1.Cities.Mumbai], Book_1.Cities[Book_1.Cities.Chennai], Book_1.Cities[Book_1.Cities.Delhi]
        ];
        this.book.BookId = 1;
        this.book.PublishYear = 2002;
        //this.book.SetIsbnNumber(524526);
        //this.book.Edition = 6;
        this.book.SetIsbnNumber();
        this.book.Add(10, 11, 12);
        console.log(this.book.Add(10, 11, 12));
        console.log(this.book.Add(10, 12));
        console.log(this.book.GetSomeData("Ram", 25));
        console.log(this.book.GetSomeData(25));
    }
    return BookComponent;
}());
BookComponent = __decorate([
    core_1.Component({
        selector: "ap-book",
        moduleId: module.id,
        templateUrl: "Book.component.html"
    }),
    __metadata("design:paramtypes", [])
], BookComponent);
exports.BookComponent = BookComponent;
//# sourceMappingURL=book.component.js.map